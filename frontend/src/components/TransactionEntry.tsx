...
import { id } from "date-fns@3.6.0/locale";
...
export function TransactionEntry({ onNavigate }: TransactionEntryProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      type: activeTab, // 'income' atau 'expense' (sesuai tab yang aktif)
      amount: parseInt(formData.amount.replace(/\D/g, '')), // Ubah "Rp 10.000" jadi angka 10000
      description: formData.description,
      category: formData.category,
      date: format(transactionDate, "dd MMM yyyy", { locale: id })
    };
    ...
    try {
      ...
      if (result.success) {
        ...
        onNavigate('dashboard'); // Kembali ke dashboard
      } else {
            ...
            <CardContent className="space-y-3">
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <Button type="submit" className="flex-1 h-8 text-xs bg-green-600 hover:bg-green-700">
                    Simpan
                  </Button>
                </div>
              </form>
